...
메일 텍스트만 출력해.`,
  ...
  /* 키워드 트렌드 리서치 (웹 검색 기반, 한도 초과 시 지식 기반 자동 폴백) */
  async researchTrend(keywords) {
    const ask = withWeb => `"${keywords}" 관련 최신 디자인/뷰티 비주얼 트렌드를 ${withWeb ? '웹에서 조사해서' : '알고 있는 지식을 바탕으로'} 한국어로 정리해줘.
형식(각 항목은 줄바꿈으로 구분):
① 지금 뜨는 흐름 3가지 — 각각 [흐름 이름] + 2문장 설명
② 리필드에 적용할 포인트 3가지 — 실행 가능한 구체안으로
③ 레퍼런스 서칭용 검색 키워드 5개 (영어 포함)
간결하게, 총 600자 내외.`;
    try {
      ...
      // 웹 검색(그라운딩)은 무료 한도가 훨씬 작음 → 한도 초과면 웹 검색 없이 재시도
      if (err.quota || /quota|한도|RESOURCE_EXHAUSTED|rate.?limit/i.test(err.message)) {
        const out = await callLLM({ system: BRAND_SYSTEM, prompt: ask(false), maxTokens: 2000 });
        return `⚠️ 웹 검색 무료 한도를 초과해서, 이번 리포트는 웹 검색 없이 AI 지식 기반으로 정리했어요. (한도는 매일 초기화돼요)\n\n${out}`;
      }
      throw err;
    }
  },
